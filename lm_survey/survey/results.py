...
class SurveyResults:
    ...
    def _calculate_D_M(self, df, by_top_completion=True, by_distribution=False):
        """Calculate the D_M metric for a dataframe, either by looking at the sampled completion (by_top_completion=True) or (TODO) by considering the average distribution generated by a language model (by_distribution=True)."""
        ...
        if by_top_completion:
            ...
            proportions = df.groupby("top_completion").agg({"weight": "sum"})
            for prop in proportions.index:
                prop_dict[prop] = (
                    proportions.loc[prop].values[0] / proportions.values.sum()
                )

        return prop_dict

    def _calculate_D_H(self, df, rdf=None, use_opinion_qa=True):
        f"""Calculate the D_H metric for a dataframe."""

        if use_opinion_qa:
            possible_completions = [
                pc.strip() for pc in df.possible_completions.iloc[0]
            ]
            weights = rdf.iloc[0].D_O
            D_H = {k: v for k, v in zip(possible_completions, weights)}
            return D_H
            pass
        else:
            possible_completions = df.possible_completions.iloc[0]
    ...
    def _get_wd(self, df):
        ...
        wave = df.wave.iloc[0]
        file_path = Path(
            f"data/ATP/distributions/American_Trends_Panel_{wave}_baseline.csv"
        )
        # How do I get the right D_H?
        rdf = pd.read_csv(file_path)
        rdf = rdf[rdf.qkey == key]
        rdf["D_O"] = rdf["D_O"].apply(lambda x: np.fromstring(x[1:-1], sep=" "))  # type: ignore

        D_H = self._calculate_D_H(df, rdf=rdf)
...
if __name__ == "__main__":
    ...
    # print(survey_results.get_representativeness(survey_results.df))
